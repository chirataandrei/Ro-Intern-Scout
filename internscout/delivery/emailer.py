...
def _section_html(title: str, jobs: list[Job], empty: str) -> str:
    heading = (
    ...
    return heading + "".join(_card_html(job) for job in jobs)


def _plain_section(title: str, jobs: list[Job], empty: str) -> list[str]:
    lines = [title, ""]
    if not jobs:
        lines.append(empty)
        lines.append("")
...
def build_email(new_jobs: list[Job], open_jobs: list[Job]) -> tuple[str, str, str]:
    """Email body lists only ``new_jobs``. ``open_jobs`` is used for counts."""
    today = datetime.now(ZoneInfo("Europe/Bucharest")).strftime("%d.%m.%Y")
    n_new = len(new_jobs)
    n_open = len(open_jobs)
    subject = f"Internships & spring weeks — {n_new} new ({today})"
    romania, remote_eu, spring = split_for_email(new_jobs)

    sections = [
        (
            "Romania internships",
            romania,
            "No new Romania internships since the last scan.",
        ),
        (
            "Remote internships (EU / EMEA)",
            remote_eu,
            "No new remote-EU internships since the last scan.",
        ),
        (
            "Spring weeks (UK / EU — not internships, not US/Canada)",
            spring,
            "No new spring weeks since the last scan.",
        ),
    ]

    plain_lines = [
        f"New internships since the last scan — {today}",
        f"New: {n_new}    Still open (already sent): {n_open}",
        "",
        "Only roles you have not been sent before.",
        "",
    ]
    if not new_jobs:
        ...
        for title, jobs, empty in sections:
            if jobs:
                plain_lines.extend(_plain_section(title, jobs, empty))
    plain = "\n".join(plain_lines)

    if not new_jobs:
        cards = ["<p style='margin:0 0 16px;color:#64748b'>Nothing new since the last scan.</p>"]
    else:
        cards = [_section_html(title, jobs, empty) for title, jobs, empty in sections if jobs]

    html_body = f"""
    <html>
      <body style="margin:0;padding:0;background:#f8fafc;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif">
        <div style="max-width:640px;margin:0 auto;padding:28px 16px">
          <h1 style="font-size:22px;margin:0 0 8px;color:#0f172a">New internships &amp; spring weeks</h1>
          <p style="margin:0 0 20px;color:#475569">
            Only postings not emailed before · Romania · remote-EU · spring weeks
          ...
          <p style="margin:28px 0 0;color:#94a3b8;font-size:12px">
            internscout digest. Already-seen roles are not included. Internships are
            Romania or remote-EU. Abroad in-office roles only keep spring / insight weeks.
          </p>
        </div>
      </body>
    </html>
